...
def build_notebooklm_package(
    ...
    """
    Build a single Markdown file optimised for upload to NotebookLM as a source.

    Structure:
      1. Portfolio overview (KPIs)
      2. Full holdings table
      3. Weighted allocation analysis
      4. Manager concentration
      5. Product-type breakdown (annuity vs capital)
      6. Presentation build guide (Israeli financial advisor framework)
      7. Suggested questions for NotebookLM
    """
    if totals is None:
        from client_portfolio.charts import compute_totals
        totals = compute_totals(df) if not df.empty else {}
    ...
    kpi_section = f"""# ניתוח תיק השקעות — {client_name or 'לקוח'}
{name_hdr}
**תאריך הפקה:** {now}
...
"""
    ...
    holdings_md = "## 2. פירוט החזקות\n\n"
    if not active.empty:
        total_amt = active["amount"].sum()
        holdings_md += "| גוף | מוצר | מסלול | סוג | סכום | משקל | מניות | חו\"ל | מט\"ח | לא סחיר | שארפ | מקור |\n"
        holdings_md += "|-----|------|-------|-----|------|------|-------|------|------|---------|------|------|\n"
        for _, h in active.iterrows():
    ...
    if not active.empty:
        ...
        mgr_section += "| מנהל | סכום | משקל |\n|------|------|------|\n"
        for _, row in mgr_grp.iterrows():
            risk = " ⚠️ ריכוז גבוה" if row["weight"] > 30 else ""
            mgr_section += f"| {row['provider']} | {_ils(row['amount'])} | {row['weight']:.1f}%{risk} |\n"

    # ── Section 4: Product type breakdown ────────────────────────────────
    _ANNUITY = {"קרנות פנסיה"}
    _CAPITAL  = {"קרנות השתלמות", "פוליסות חיסכון", "קופות גמל", "גמל להשקעה"}
    prod_section = "\n## 4. פיזור בין סוגי מוצרים\n\n"
    if not active.empty and "product_type" in active.columns:
        ...
        prod_section += "| סוג מוצר | קטגוריה | סכום | משקל |\n|---------|---------|------|------|\n"
        for _, row in prod_grp.sort_values("amount", ascending=False).iterrows():
            prod_section += f"| {row['product_type']} | {row['category']} | {_ils(row['amount'])} | {row['weight']:.1f}% |\n"

    # ── Section 5: Presentation build guide ──────────────────────────────
    guide = f"""
---

## 5. מדריך בניית מצגת ללקוח

### עקרונות מנחים

מצגת ייעוץ השקעות מקצועית לישראל צריכה לעמוד בעקרונות הבאים:

1. **פשטות ובהירות** — לקוח לא מומחה צריך להבין בעמוד אחד את המצב הנוכחי שלו
2. **מספרים בולטים** — סך הנכסים, המנהלים, והתמהיל המשוקלל — בגדול ובמרכז
3. **השוואה ל-benchmark** — מה ממוצע השוק? איפה הלקוח ביחס אליו?
4. **תמונה גרפית** — עוגת הפיזור בין המנהלים ועוגת הנכסים — חובה בכל מצגת
5. **סיכון ונזילות** — להבליט את החשיפה ללא-סחיר ומה משמעותה
6. **עלויות** — להראות את העלות המשוקללת ביחס לשוק
7. **המלצה ברורה** — כל מצגת מסתיימת ב-3 נקודות פעולה ספציפיות

### מבנה מצגת מומלץ (8 שקפים)

**שקף 1 — נתוני הלקוח:**
- שם, תאריך, סך נכסים בולט
- מספר מוצרים ומנהלים

**שקף 2 — סיכום מנהלים:**
- עוגת פיזור בין מנהלים
- אם יש מנהל עם >30%: הדגשה אדומה

**שקף 3 — תמהיל נכסים:**
- עוגות: מניות/אגח, חול/ישראל, מטח/שקל
- עם השוואה לממוצע השוק

**שקף 4 — פיזור מוצרים:**
- קצבה vs הון
- רלוונטי לגיל הלקוח ולצרכיו

**שקף 5 — ניתוח סיכון:**
- חשיפה ללא-סחיר (סיכון נזילות)
- תנודתיות (שארפ)
- ריכוז (HHI)

**שקף 6 — עלויות:**
- דמי ניהול לכל מוצר
- עלות שנתית כוללת vs חיסכון פוטנציאלי

**שקף 7 — השוואה לאלטרנטיבות:**
- תמהיל מוצע מהאופטימייזר
- שיפור פוטנציאלי בשארפ, עלות, ופיזור

**שקף 8 — המלצות:**
- 3 פעולות ספציפיות
- סדר עדיפויות וציר זמן

### שאלות מומלצות לשאול את NotebookLM

1. "סכם את מצב התיק של הלקוח ב-3 משפטים"
2. "מהם הסיכונים העיקריים בתיק זה?"
3. "האם יש ריכוז יתר אצל מנהל מסוים?"
4. "כיצד לשפר את הפיזור בין מנהלים בתיק זה?"
5. "מה העלות השנתית הכוללת ואיך ניתן להפחיתה?"
6. "האם החשיפה למניות מתאימה לפרופיל סיכון ממוצע?"
7. "כתוב טקסט פתיחה לשקף הראשון של המצגת ללקוח {client_name or 'זה'}"
8. "מה ההמלצות הטופ-3 לשיפור התיק?"

---

## 6. נתוני גלם לניתוח נוסף

```json
PORTFOLIO_TOTALS: {{
  "total_amount": {totals.get('total', 0):.0f},
  "n_products": {totals.get('n_products', 0)},
  "n_managers": {totals.get('n_managers', 0)},
  ...
  "annual_cost_pct_weighted": {totals.get('cost', 0) or 0:.3f},
  "report_date": "{now}"
}}
```
...
"""
    ...
    full_doc = kpi_section + holdings_md + mgr_section + prod_section + guide
